...
import pandas as pd
import numpy as np
from components_duck.db_postgres import duck_query
from datetime import date, timedelta
import plotly.express as px
from streamlit_extras.stylable_container import stylable_container
from components_duck.alertas_duck import render_alertas_duck
from components_duck.utils_duck import (
    criar_gauge_plot,
...
from components_duck.db_lojas_duck import (
...
# Adicione as funções de navegação para alertas (após as outras funções auxiliares):
...
def render():
    ...
    st.markdown("""
        <style>
        /* Popovers dos KPIs: fundo cinza claro, barra superior e altura igual ao ALERTAS (108px) */
        ...
        white-space: pre-line !important; /* permite quebras de linha */
        transition: all 0.3s ease !important;
        cursor: pointer !important;
        
        }

        /* Barra superior mais escura */
        ...
        /* Título menor (primeiro parágrafo) */
        ...
        /* Centraliza o conteúdo do Markdown */
        ...
        /* Hover sutil (mantém cinza claro) */
        ...
        /* Evita que temas alterem a borda/cores no hover */
        button[data-testid="stPopoverButton"]:hover,
        button[data-testid="stPopoverButton"]:focus {
        outline: none !important;
        }

        /* Garante consistência após recarregar filtros */
        button[data-testid="stPopoverButton"] * {
        color: inherit !important;
        }
                
        /* Força largura mínima do conteúdo dentro do popover */
        div[role="dialog"][data-testid="stPopover"] [data-baseweb="tab-panel"] {
            min-width: 650px !important;
            width: 100% !important;
        }
        
        /* Força largura mínima para gráficos/tabelas dentro do popover */
        div[role="dialog"][data-testid="stPopover"] [data-testid="element-container"] {
            min-width: 650px !important;
            width: 100% !important;
        }
        
        /* Força largura mínima para os elementos de visualização (gráficos) */
    ...
    # Bounds de data (pode ser útil depois)
    ...
    # Filtro de Filial (com 'Todas as Filiais')
    df_lojas = duck_query("""
        SELECT DISTINCT 
    ...
    # Linha 1: 8 Gauges
    venda_acumulada = venda_acumulada_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )
    d1_val = venda_dezena1_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )
    d2_val = venda_dezena2_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )
    d3_val = venda_dezena3_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )
    cmv_atual = cmv_percent_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )
    conversao_atual = conversao_vitaminas_percent_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )
    desconto_atual = desconto_percent_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )
    vendas_identificadas_atual = vendas_identificadas_percent_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )
    ...
    with col2:
        st.plotly_chart(criar_gauge_plot(d1_val, meta_d1, "1ª Dezena (1-10)"), use_container_width=True, config=cfg)
    with col3:
        st.plotly_chart(criar_gauge_plot(d2_val, meta_d2, "2ª Dezena (11-20)"), use_container_width=True, config=cfg)
    with col4:
        st.plotly_chart(criar_gauge_plot(d3_val, meta_d3, "3ª Dezena (21-Fim)"), use_container_width=True, config=cfg)
    with col5:
        st.plotly_chart(criar_gauge_invertido_plot(cmv_atual, meta_cmv, "CMV (%)"), use_container_width=True, config=cfg)
    with col6:
        st.plotly_chart(criar_gauge_plot(conversao_atual, meta_conversao, "Conversão Vitaminas (%)"), use_container_width=True, config=cfg)
    with col7:
        st.plotly_chart(criar_gauge_invertido_plot(desconto_atual, meta_desconto, "Desconto (%)"), use_container_width=True, config=cfg)
    with col8:
        st.plotly_chart(criar_gauge_plot(vendas_identificadas_atual, meta_identificadas, "Vendas Identif. (%)"), use_container_width=True, config=cfg)

    # Helper para card vazio (mesma aparência via utils.render_kpi_card)
    def card_vazio(label, valor="—", perc=None):
        from components.utils import render_kpi_card
        render_kpi_card(label, valor, perc)

    # KPIs para os cards (linha 2)
    num_vendas = num_vendas_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )
    ticket_medio = (venda_acumulada / num_vendas) if num_vendas > 0 else 0.0

    # Linha 2: 4 cards (preenche 3 primeiros)
    c1, c2, c3, c4 = st.columns(4, gap='small')
    with c1:
        # Adicione um ID único no container
        st.markdown('<div id="venda-acumulada-popover-wrapper">', unsafe_allow_html=True)
        ...
        with st.popover(label=label, use_container_width=True, help="Clique para ver detalhes"):
            render_popover_vendas_acumuladas(None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado))
        st.markdown('</div>', unsafe_allow_html=True)
        # CSS específico para este ID
        st.markdown("""
        <style>
        #venda-acumulada-popover-wrapper button[kind="secondary"][data-testid="stPopoverButton"] {
        ...
        """, unsafe_allow_html=True)
    with c2:
        st.markdown('<div id="num-vendas-popover-wrapper">', unsafe_allow_html=True)
        label = f"🛒 NÚMERO DE VENDAS\n\n{formatar_numero(num_vendas)}"
        with st.popover(label=label, use_container_width=True, help="Clique para ver detalhes"):
            render_popover_num_vendas(None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado))
        st.markdown('</div>', unsafe_allow_html=True)
        # CSS específico para este ID
        st.markdown("""
        <style>
        #num-vendas-popover-wrapper button[kind="secondary"][data-testid="stPopoverButton"] {
            background:#f5f7fa !important;
            /* outros estilos */
        }
        </style>
    ...
    with c3:
        st.markdown('<div id="ticket-medio-card-wrapper">', unsafe_allow_html=True)
        label = f"🧾 TICKET MÉDIO\n\n{formatar_moeda(ticket_medio)}"
        with st.popover(label=label, use_container_width=True, help="Clique para ver detalhes"):
            render_popover_ticket_medio(None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado))
        st.markdown('</div>', unsafe_allow_html=True)
        # CSS específico para este ID
        st.markdown("""
        <style>
        #ticket-medio-card-wrapper button[kind="secondary"][data-testid="stPopoverButton"] {
            background:#f5f7fa !important;
            /* outros estilos */
        }
        </style>
    ...
    with c4:
        st.markdown('<div id="top-produtos-popover-wrapper">', unsafe_allow_html=True)
        label = f"🏆 TOP 10 PRODUTOS"
        with st.popover(label=label, use_container_width=True, help="Clique para ver detalhes"):
            render_popover_top_produtos(None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado))
        st.markdown('</div>', unsafe_allow_html=True)
        # CSS específico para este ID
        st.markdown("""
        <style>
        #top-produtos-popover-wrapper button[kind="secondary"][data-testid="stPopoverButton"] {
            background:#f5f7fa !important;
            /* outros estilos */
        }
        </style>
    ...
    # KPIs para os cards (linha 3)
    itens_por_nota = itens_por_nota_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )
    vitaminas_vendidas = vitaminas_vendidas_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )
    taxa_conv = conversao_vitaminas_percent_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )

    c5, c6, c7, c8 = st.columns(4, gap='small')
    with c5:
        st.markdown('<div id="itens-por-nota-popover-wrapper">', unsafe_allow_html=True)
        label = f"📦 ITENS POR NOTA\n\n{formatar_numero_uma_casa(itens_por_nota)}"
        with st.popover(label=label, use_container_width=True, help="Clique para ver detalhes"):
            render_popover_itens_por_nota(None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado))
        st.markdown('</div>', unsafe_allow_html=True)
        # CSS específico para este ID
        st.markdown("""
        <style>
        #itens-por-nota-popover-wrapper button[kind="secondary"][data-testid="stPopoverButton"] {
            background:#f5f7fa !important;
            /* outros estilos */
        }
        </style>
    ...
    with c6:
        st.markdown('<div id="vitaminas-vendidas-popover-wrapper">', unsafe_allow_html=True)
        label = f"💊 VITAMINAS VENDIDAS\n\n{formatar_numero(vitaminas_vendidas)}"
        with st.popover(label=label, use_container_width=True, help="Clique para ver detalhes"):
            render_popover_vitaminas_vendidas(None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado))
        st.markdown('</div>', unsafe_allow_html=True)
        # CSS específico para este ID
        st.markdown("""
        <style>
        #vitaminas-vendidas-popover-wrapper button[kind="secondary"][data-testid="stPopoverButton"] {
            background:#f5f7fa !important;
            /* outros estilos */
        }
        </style>
    ...
    with c7:
        st.markdown('<div id="conversao-vitaminas-popover-wrapper">', unsafe_allow_html=True)
        label = f"🔄 TAXA DE CONVERSÃO\n\n{formatar_numero_uma_casa(taxa_conv)}"
        with st.popover(label=label, use_container_width=True, help="Clique para ver detalhes"):
            render_popover_conversao_vitaminas(None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado))
        st.markdown('</div>', unsafe_allow_html=True)
        # CSS específico para este ID
        st.markdown("""
        <style>
        #conversao-vitaminas-popover-wrapper button[kind="secondary"][data-testid="stPopoverButton"] {
            background:#f5f7fa !important;
            /* outros estilos */
        }
        </style>
        """, unsafe_allow_html=True)
    with c8:
        st.markdown('<div id="analise-dia-hora-popover-wrapper">', unsafe_allow_html=True)
        label = f"⏰ ANÁLISE DIA/HORA"
        with st.popover(label=label, use_container_width=True, help="Clique para ver detalhes"):
            render_popover_analise_dia_hora(None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado))
        st.markdown('</div>', unsafe_allow_html=True)
        # CSS específico para este ID
        st.markdown("""
        <style>
        #analise-dia-hora-popover-wrapper button[kind="secondary"][data-testid="stPopoverButton"] {
            background:#f5f7fa !important;
            /* outros estilos */
        }
        </style>
    ...
    # Linha 4: 4 cards — variante “alt” (cinza esverdeado)
    c9, c10, c11, c12 = st.columns(4, gap='small')
    with c9:
        st.markdown('<div id="cmv-popover-wrapper">', unsafe_allow_html=True)
        label = f"📉 CMV (%)\n\n{formatar_numero_uma_casa(cmv_atual)}%"
        with st.popover(label=label, use_container_width=True, help="Clique para ver detalhes"):
            render_popover_cmv(None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado))
        st.markdown('</div>', unsafe_allow_html=True)
        # CSS específico para este ID
        st.markdown("""
        <style>
        #cmv-popover-wrapper button[kind="secondary"][data-testid="stPopoverButton"] {
            background:#f5f7fa !important;
            /* outros estilos */
        }
        </style>
        """, unsafe_allow_html=True)
    with c10:
        st.markdown('<div id="desconto-popover-wrapper">', unsafe_allow_html=True)
        label = f"🏷️ DESCONTO (%)\n\n{formatar_percentual(desconto_atual)}"
        with st.popover(label=label, use_container_width=True, help="Clique para ver detalhes"):
            render_popover_descontos(None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado))
        st.markdown('</div>', unsafe_allow_html=True)
        # CSS específico para este ID
        st.markdown("""
        <style>
        #desconto-popover-wrapper button[kind="secondary"][data-testid="stPopoverButton"] {
            background:#f5f7fa !important;
            /* outros estilos */
        }
        </style>
        """, unsafe_allow_html=True)
    with c11:
        st.markdown('<div id="vendas-identificadas-popover-wrapper">', unsafe_allow_html=True)
        label = f"🆔 VENDAS IDENTIFICADAS (%)\n\n{formatar_percentual(vendas_identificadas_atual)}"
        with st.popover(label=label, use_container_width=True, help="Clique para ver detalhes"):
            render_popover_vendas_identificadas(None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado))
        st.markdown('</div>', unsafe_allow_html=True)
        # CSS específico para este ID
        st.markdown("""
        <style>
        #vendas-identificadas-popover-wrapper button[kind="secondary"][data-testid="stPopoverButton"] {
            background:#f5f7fa !important;
            /* outros estilos */
        }
        </style>
        """, unsafe_allow_html=True)
    # No Card 12 (ALERTAS) - substitua o código atual por:
    with c12:
        # ALERTAS com a MESMA largura/altura dos demais cards (mantém vermelho)
        with stylable_container(
    key="alertas_kpi_card_duck",  # chave nova para fugir de regras antigas
    css_styles="""
            .stButton > button {
                width: 100% !important;
                max-width: 100% !important;
    ...
    #     st.markdown("""
    # <style>
    #   .st-key-alertas_kpi_card_duck > div:first-child { margin:0 !important; padding:0 !important; }
    # </style>
    # """, unsafe_allow_html=True)
    
    # Linha 5: Classificação N1 (acumulado do mês)
    df_cls = vendas_por_classificacao_n1_mes_atual(
        None if codigo_selecionado == "Todas as Filiais" else str(codigo_selecionado)
    )

    # Dicionários de configuração (iguais ao lojas.py)
    ...
    # Mapas a partir do SQL (já vem UPPER no SELECT)
    ...
    for i, classificacao in enumerate(ordem_classificacao):
        with cols[i]:
            ...
            # Lógica da barra (mesma do lojas.py)
